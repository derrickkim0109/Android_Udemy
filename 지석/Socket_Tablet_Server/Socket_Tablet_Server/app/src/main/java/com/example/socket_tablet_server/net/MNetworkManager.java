...
public class MNetworkManager {
    ...
    public MNetworkManager(String _ip, int _port, ITF_Network _itf, Handler _handler) {
        ...
        mReadSleepTime = TAG_NET.READ_SLEEP_TIME;
    }

    /**
     * mSetNetworkReadSleepTime
     *
     * @param _time
     */
    ...
    private class MSocketInitThread extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                mIsConnect = false;
                //Dlog.e("mIsConnect is false : " + e.toString());
                mSocket = null;
                //Dlog.d("Error MSocketInitThread : " + e.toString());
                mHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        itfNetwork.mConnectFail();
                    }
                });
            }
        }
...
//                        Dlog.d("MReadThread _read_len : " + _read_len);
                        if (_read_len == -1) {
                        } else {
                            //Dlog.d("MReadThread read str : " + new String(_read_buf, 0, _read_len));
                            itfNetwork.mReadComplete(new String(_read_buf, 0, _read_len));
                        }
                        sleep(mReadSleepTime);
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    private class MCloseThread extends Thread {
        @Override
        public void run() {
